#==================================================================================
# Script: 	Get-PoolAvailabilityState.ps1
# Date:		01/08/18
# Author: 	Andi Patrick
...
$SCRIPT_NAME			= 'Get-PoolAvailabilityState.ps1'
$EVENT_LEVEL_ERROR 		= 1
...
# bigipTrafficMgmt.bigipLocalTM.ltmPools.ltmPoolStatus.ltmPoolStatusTable.ltmPoolStatusEntry.ltmPoolStatusAvailState
[string]$ltmPoolStatusAvailState = ".1.3.6.1.4.1.3375.2.2.5.5.2.1.2"
...
For ($i=0;$i -lt $DeviceAddressList.Count;$i++) {
	Try {
		...
		# Get Pool AvailabilityState from SNMP
		$PoolAvailabilityStateList = New-Object 'System.Collections.Generic.List[Lextm.SharpSnmpLib.Variable]'
		[Lextm.SharpSnmpLib.Messaging.Messenger]::Walk($ver, $svr, $DeviceCommunityList[$i], $ltmPoolStatusAvailState , $PoolAvailabilityStateList, 3000, $walkMode)
		...
		For($i=0; $i -lt $PoolNameList.Count; $i++) {
			[int]$PoolEnabledState = $PoolEnabledStateList[$i].Data.ToString()

			# Only do Enabled Pools
			If ($PoolEnabledState -eq 1) {
				[string]$PoolName = $PoolNameList[$i].Data.ToString()
				[int]$PoolAvailabilityState = $PoolAvailabilityStateList[$i].Data.ToString()
				[string]$PoolDetailedReason = $PoolDetailedReasonList[$i].Data.ToString()

				#Create a property bag.
				[string] $message = "Creating Property bag for : " + $PoolName + "`r`nAvailability State : " + $PoolAvailabilityState + "`r`nDetailed Reason : " + $PoolDetailedReason
				Log-DebugEvent $SCRIPT_PROPERTYBAG_CREATED $message
				$bag = $api.CreatePropertyBag()
				$bag.AddValue("PoolName", $PoolName)
				$bag.AddValue("AvailabilityState", $PoolAvailabilityState)
